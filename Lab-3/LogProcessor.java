...
class LogProcessorFast implements LogProcessor {
  ...
  public void add(String url, int time) {
	...
	queue.add(pt);
	while (queue.lastEntry().getElement().time - queue.firstEntry().getElement().time > W) {
  ...
  public List<String> getOrderedUrlsInWindow(int K) {
	List<String> result = new ArrayList<String>();
	urlToCount = new HashMap<String,PageCount>();
	for ( PageTime i : queue ) {
	  if ( urlToCount.containsKey(i.url)) {
		int count = urlToCount.get(i.url).count;
		PageCount pt = new PageCount(i.url,count + 1);
	    urlToCount.put(i.url, pt);
	  } else {
	    urlToCount.put(i.url, new PageCount(i.url,1));
	  }
	}
	counts = new TreeSet<PageCount>();
	for(PageCount pci : urlToCount.values()){
		counts.add((PageCount)pci);
	}
	
	for(PageCount pcj : counts){
		if(K > 0){
			result.add( pcj.url + ":" + pcj.count);
			K--;
		}
	}
